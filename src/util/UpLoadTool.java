...
public class UpLoadTool {
	//上传图片的相对路径
	public String upload_path = "upload/headPic/";
	//设置上传文件最大10k
	public int kb = 10;
	//允许上传的文件格式列表,默认有三种jpg,mpg和gif
	...
	//设置可以上传的格式列表
	public void setAllowedExt(String [] ext){
		for(int i=0;i<ext.length;i++){
			allowedExt.add(ext[i]);
		}
	}
				...
				if(item_file != null){
					...
					if(!"".equals(fileName) && size != 0) {
						...
						if(allowFlag == allowedExtCount){
							out.print(
							"<script>alert('上传的图片格式不对！');window.location='register.jsp';</script>");
							return ;
						}
					}
