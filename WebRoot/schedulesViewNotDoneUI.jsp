...
<%
	...
	List <ScheduleDTO> list = dao.findAllFinishScheduleInBatch(((pages-1)*limit),limit,user_id,0);
			...
			<div class="row">
				...
				<div class="span9">
					...
					<!-- 分页模块begin -->
						<div width="50%" style="margin-top:5px;text-align:right;float:right;">
							<form name="f1" method="post" action="schedulesViewNotDone.jsp" onSubmit="return checkNum(<%=pageTotal%>);">	
								第<%=pages%>页&nbsp;&nbsp;
								共<%=pageTotal%>页&nbsp;&nbsp;
								<a href="schedulesViewNotDoneUI.jsp?pages=1">首页</a>&nbsp;&nbsp;
								<a href="schedulesViewNotDoneUI.jsp?pages=<%=(pages<1)?pages:(pages-1)%>" onclick="return checkDoLastPage(<%=pages %>);">上一页</a>&nbsp;&nbsp;
								<a href="schedulesViewNotDoneUI.jsp?pages=<%=(pages>pageTotal)?pages:(pages+1)%>" onclick="return checkDoNextPage(<%=pages%>,<%=pageTotal%>);">下一页</a>&nbsp;&nbsp;
								<a href="schedulesViewNotDoneUI.jsp?pages=<%=pageTotal%>">最后一页</a>&nbsp;&nbsp;
								转到第<input type="number" max="<%=pageTotal %>" min="1" step="1" id="pages" name="pages" style="width:60px" />页&nbsp;&nbsp;
					...
					<form id="new-task1"  method="post" action="jspDo/schedulesAddDo.jsp">
						<fieldset>
							<p><span style="font-weight:bold;font-size:1.5em">添加日程</span></p>
							<div class="control-group">
								<p><span style="font-weight:bold">日程名称：</span></p>
								<input  type="text" name="sch_name" id="sch_name" />
							</div>
							<div class="control-group">
								<p><span style="font-weight:bold">日程说明：</span></p>
								<div class="controls">
